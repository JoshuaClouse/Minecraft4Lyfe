...
makes the program run much faster. Also, if more of an item is required by our goal, the heuristic will change as such. 
If we ever had more than the maximum amount of an item, we would make that item then have a cost of 50000000 so that it would be 
very unlikely to be considered again in the A* algorithm. If you used an item to craft another item, then the item count 
